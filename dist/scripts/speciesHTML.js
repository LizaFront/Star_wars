...
const getHTMLContentSpecies = (data, planet) => {
    if (!data || data.hasOwnProperty('detail')) {
        return errorContent();
    }
    if ('skin_colors' in data) {
        const { name, language, skin_colors, hair_colors, eye_colors, designation, classification, average_lifespan, average_height, films, } = data;
