...
const showMainContainer = (): void => {
	if (mainContainer) {
		mainContainer.style.visibility = 'visible';
	}
};
const hideMainContainer = (): void => {
	if (mainContainer) {
...
const getPlanetName = async (data: DataType): Promise<string> => {
	if (!data) {
		return errorContent();
	}
...
const closeMainContainer = (elem: HTMLElement) => {
...
const resultforSearch = async (
	fetch: (url: string) => Promise<IData>,
	HTMLFunction: (data: DataType, planet?: string) => string,
	getPlanetNameFun?: (data: DataType) => Promise<string>
) => {
	try {
	...
	const close: HTMLElement | null = document.querySelector('.message-header');
	if (close) {
...
const resultforGetById = async (
	fetch: (url: string) => Promise<DataType>,
	HTMLFunction: (data: DataType) => string
) => {
	try {
		const result = await fetch(search[1].value);
		if (result) {
			hideLoader();
			if (mainContainer) mainContainer.innerHTML = HTMLFunction(result);
			showMainContainer();
		}
	} catch (error) {
		hideLoader();
	...
	const close: HTMLElement | null = document.querySelector('.message-header');
	if (close) closeMainContainer(close);
};
